...
/// This materializes ceildiv(dimX * dimY * dimZ, subgroup_size).
struct LowerNumSubgroupsOp final : OpRewritePattern<gpu::NumSubgroupsOp> {
  ...
  LogicalResult matchAndRewrite(gpu::NumSubgroupsOp op,
    ...
    auto getBlockDim = [&](gpu::Dimension dim) -> Value {
      auto maybeKnownDim =
          gpu::getKnownDimensionSizeAround(op, gpu::DimensionKind::Block, dim);
      if (IntegerAttr knownDim = asMaybeIndexAttr(maybeKnownDim)) {
    ...
    // The `upper_bound` on `gpu.num_subgroups` bounds this quotient, not
    // subgroup size, so there is no equivalent attribute to forward to
    // `gpu.subgroup_size`.
    Value subgroupSize = gpu::SubgroupSizeOp::create(
        rewriter, loc, indexType, /*upper_bound=*/nullptr);

    Value xy = arith::MulIOp::create(rewriter, loc, dimX, dimY);
    Value xyz = arith::MulIOp::create(rewriter, loc, xy, dimZ);
    Value numSubgroups =
        arith::CeilDivUIOp::create(rewriter, loc, xyz, subgroupSize);
