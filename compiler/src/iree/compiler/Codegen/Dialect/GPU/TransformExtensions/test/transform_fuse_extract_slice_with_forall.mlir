...
module {
  module {
    func.func @fuse_dynamic_extract_slice_with_forall(%arg0: tensor<?x?xf32>, %arg1: index, %arg2: index, %arg3: index, %arg4: index, %arg5: index, %arg6: index) -> tensor<?x?xf32> {
  ...
  module attributes {transform.with_named_sequence} {
    transform.named_sequence @__transform_main(%arg0: !transform.any_op {transform.readonly}) {
      ...
      %2 = transform.iree.fuse_extract_slice_with_forall %1 into %0 : (!transform.any_op, !transform.any_op) -> !transform.any_op
      transform.yield
    }
  }
}
...
// CHECK-LABEL: func @fuse_dynamic_extract_slice_with_forall
...
//   CHECK-DAG:     %[[OUT_SLICE:.+]] = tensor.extract_slice %[[SLICED_BBARG]]
//  CHECK-SAME:       [%[[IDX0]], %[[IDX1]]] [%[[SIZE_CLAMPED_LOW0]], %[[SIZE_CLAMPED_LOW1]]] [1, 1] : tensor<?x?xf32> to tensor<?x?xf32>
//   CHECK-DAG:     %[[PAD_HIGH0:.+]] = affine.apply #[[$MAP2]](%[[SIZE_CLAMPED_LOW0]])
//   CHECK-DAG:     %[[PAD_HIGH1:.+]] = affine.apply #[[$MAP2]](%[[SIZE_CLAMPED_LOW1]])
//       CHECK:     %[[PADDED_OUT_SLICE:.+]] = tensor.pad %[[OUT_SLICE]] low[0, 0] high[%[[PAD_HIGH0]], %[[PAD_HIGH1]]] {
//       CHECK:     ^bb0({{.*}}):
//       CHECK:       tensor.yield %[[ZERO]] : f32
//       CHECK:     } : tensor<?x?xf32> to tensor<?x?xf32>
//       CHECK:     %[[COPY:.+]] = linalg.copy ins(%[[IN_SLICE]] : tensor<?x?xf32>) outs(%[[PADDED_OUT_SLICE]] : tensor<?x?xf32>) -> tensor<?x?xf32>
...
module {
  func.func @fuse_rank_reduced_extract_slice_with_forall(%arg0: tensor<4x8xf32>, %arg1: index) -> tensor<?xf32> {
...
// CHECK-LABEL: func @fuse_rank_reduced_extract_slice_with_forall
...
//   CHECK-DAG:     %[[SIZE_CLAMPED_LOW1:.+]] = affine.max #[[$MAP1]](%[[SIZE_CLAMPED_HIGH1]])
//   CHECK-DAG:     %[[OUT_SLICE:.+]] = tensor.extract_slice %[[SLICED_BBARG]]
//  CHECK-SAME:       [%[[IDX0]], %[[IDX1]]] [%[[SIZE_CLAMPED_LOW0]], %[[SIZE_CLAMPED_LOW1]]] [1, 1] : tensor<1x?xf32> to tensor<?x?xf32>
//   CHECK-DAG:     %[[PAD_HIGH0:.+]] = affine.apply #[[$MAP3]](%[[SIZE_CLAMPED_LOW0]])
//   CHECK-DAG:     %[[PAD_HIGH1:.+]] = affine.apply #[[$MAP3]](%[[SIZE_CLAMPED_LOW1]])
//       CHECK:     %[[PADDED_OUT_SLICE:.+]] = tensor.pad %[[OUT_SLICE]] low[0, 0] high[%[[PAD_HIGH0]], %[[PAD_HIGH1]]] {
//       CHECK:     ^bb0({{.*}}):
//       CHECK:       tensor.yield %[[ZERO]] : f32
//       CHECK:     } : tensor<?x?xf32> to tensor<2x2xf32>
//       CHECK:     %[[COPY:.+]] = linalg.copy ins(%[[IN_SLICE]] : tensor<2x2xf32>) outs(%[[PADDED_OUT_SLICE]] : tensor<2x2xf32>) -> tensor<2x2xf32>
...
#map = affine_map<(d0) -> (d0 * 2)>
module {
  func.func @fuse_extract_slice_with_rank_reduced_forall_slices(%arg0: tensor<4x8xf32>, %size1: index) -> tensor<4x?xf32> {
    %0 = tensor.empty() : tensor<4x8xf32>
...
// CHECK-LABEL: func @fuse_extract_slice_with_rank_reduced_forall_slices
//  CHECK-SAME:   %[[ARG0:[A-Za-z0-9]+]]: tensor<4x8xf32>
//  CHECK-SAME:   %[[ARG1:[A-Za-z0-9]+]]: index
...
//   CHECK-DAG:     %[[OUT_SLICE:.+]] = tensor.extract_slice %[[SLICED_BBARG]]
//  CHECK-SAME:       [0, %[[SLICE_IDX]]] [1, %[[SIZE_CLAMPED_LOW]]] [1, 1] : tensor<4x?xf32> to tensor<?xf32>
//   CHECK-DAG:     %[[PAD_HIGH:.+]] = affine.apply #[[$MAP3]](%[[SIZE_CLAMPED_LOW]])
//       CHECK:     %[[PADDED_OUT_SLICE:.+]] = tensor.pad %[[OUT_SLICE]] low[0] high[%[[PAD_HIGH]]] {
//       CHECK:     ^bb0({{.*}}):
//       CHECK:       tensor.yield %[[ZERO]] : f32
...
#map = affine_map<(d0) -> (d0 * 2)>
module {
  func.func @fuse_extract_slice_with_multi_result_forall(%arg0: tensor<8xf32>, %arg1: tensor<8xf32>, %arg2: index) -> (tensor<?xf32>, tensor<8xf32>) {
    %0 = tensor.empty() : tensor<8xf32>
...
// CHECK-LABEL: func @fuse_extract_slice_with_multi_result_forall
