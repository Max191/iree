...
static FailureOr<Value> createSharedAllocDestination(RewriterBase &rewriter,
  ...
  auto hintOp =
      dyn_cast<IREE::Codegen::AllocationHintOpInterface>(destination);
  if (hintOp) {
  ...
  if (hintOp) {
    ...
    auto clonedHint =
        cast<IREE::Codegen::AllocationHintOpInterface>(cloned);
...
// AllocationHintOpInterface. This allows the patterns to work generically
// with any hint op that implements the interface.
...
struct FoldAllocationHintOpWithReshapeOp final
    : OpRewritePattern<ReshapeOp> {
  using OpRewritePattern<ReshapeOp>::OpRewritePattern;
  LogicalResult matchAndRewrite(ReshapeOp reshapeOp,
                                PatternRewriter &rewriter) const override {
    auto hintOp =
        dyn_cast_or_null<IREE::Codegen::AllocationHintOpInterface>(
