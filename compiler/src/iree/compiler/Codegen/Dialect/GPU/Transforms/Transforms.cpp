...
static LogicalResult
canClampParallelInsertSlice(RewriterBase &rewriter,
                            tensor::ParallelInsertSliceOp parallelInsertOp,
...
clampParallelInsertSliceOp(RewriterBase &rewriter,
  ...
  // dimensions will never be potentially zero by constructions. The earlier
...
fuseExtractSliceIntoProducerForall(RewriterBase &rewriter,
  ...
  std::optional<llvm::SmallDenseSet<unsigned int>>
      maybeInsertRankReductionMask = computeRankReductionMask(
          parallelInsertOp.getStaticSizes(),
          parallelInsertOp.getSourceType().getShape(), /*matchDynamic=*/true);
  ...
  // extractSliceOp.
  if (failed(canClampParallelInsertSlice(
          rewriter, parallelInsertOp, extractSliceOp,
  ...
  Value forallOutput =
      forallOp.getOutputs()[forallResult.getResultNumber()];
