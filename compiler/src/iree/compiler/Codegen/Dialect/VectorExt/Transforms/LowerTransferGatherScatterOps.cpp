...
computeUnrollDim0Maps(ArrayRef<AffineMap> indexingMaps, int64_t numIndexVecs,
  ...
  for (auto [j, expr] : llvm::enumerate(baseMap.getResults())) {
    std::optional<int64_t> coefficient =
        mlir::iree_compiler::getCoefficient(expr, 0);
    if (!coefficient) {
      return failure();
    }
    if (*coefficient != 0) {
...
struct LowerRank1TransferScatterToVectorScatter final
  ...
  LogicalResult matchAndRewrite(TransferScatterOp op,
    ...
    // This pattern linearizes through concrete memory layout metadata. Tensor
    // transfer_scatter forms with strict last-symbol maps are handled by the
    // transfer_scatter-to-vector pass; more general tensor affine forms wait
    // until bufferization exposes a memref base.
    ...
    for (int64_t dim = 0; dim < baseType.getRank(); ++dim) {
      Value dimIndexVector = materializeRank1IndexExpr(
    ...
    // This preserves the transfer_scatter expectation that active lanes do not
    // collide. vector.scatter has undefined behavior for duplicate indices.
