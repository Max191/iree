...
//       CHECK:   %[[R0:.+]] = vector.transfer_read %[[INPUT]]{{.*}}, %[[CST]] {in_bounds = [true]} : tensor<2x34x34x640xf32>, vector<4xf32>
//       CHECK:   vector.transfer_write %[[R0]], {{.*}} : vector<4xf32>, tensor<2x2x4xf32>
//       CHECK:   %[[R1:.+]] = vector.transfer_read %[[INPUT]]{{.*}}, %[[CST]] {in_bounds = [true]} : tensor<2x34x34x640xf32>, vector<4xf32>
//       CHECK:   vector.transfer_write %[[R1]], {{.*}} : vector<4xf32>, tensor<2x2x4xf32>
//       CHECK:   %[[R2:.+]] = vector.transfer_read %[[INPUT]]{{.*}}, %[[CST]] {in_bounds = [true]} : tensor<2x34x34x640xf32>, vector<4xf32>
//       CHECK:   vector.transfer_write %[[R2]], {{.*}} : vector<4xf32>, tensor<2x2x4xf32>
//       CHECK:   %[[R3:.+]] = vector.transfer_read %[[INPUT]]{{.*}}, %[[CST]] {in_bounds = [true]} : tensor<2x34x34x640xf32>, vector<4xf32>
...
// Low-side input padding on the vectorized input dimension: not supported.
func.func @im2col_no_vectorize_channel_pad_low(
    %input: tensor<59x91x16x56xbf16>, %output: tensor<1x1x1x8xbf16>
) -> tensor<1x1x1x8xbf16> {
  %cst = arith.constant 0.000000e+00 : bf16
  %c5 = arith.constant 5 : index
...
// CHECK-LABEL: func.func @im2col_no_vectorize_channel_pad_low
//       CHECK:   iree_linalg_ext.im2col
//   CHECK-NOT:   vector.transfer_read
