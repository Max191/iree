...
normalizeBroadcastOperands(arith::CmpIOp cmpOp) {
  ...
  if (auto bc = lhs.getDefiningOp<vector::BroadcastOp>()) {
    if (bc.getSource().getType().isIntOrIndex()) {
      P swapped;
      switch (pred) {
      case P::eq: swapped = P::eq; break;
      case P::ne: swapped = P::ne; break;
      case P::slt: swapped = P::sgt; break;
      case P::sle: swapped = P::sge; break;
      case P::sgt: swapped = P::slt; break;
      case P::sge: swapped = P::sle; break;
      case P::ult: swapped = P::ugt; break;
      case P::ule: swapped = P::uge; break;
      case P::ugt: swapped = P::ult; break;
      case P::uge: swapped = P::ule; break;
      default:
...
static void simplifyDivisibleCmpI(IRRewriter &rewriter, arith::CmpIOp cmpOp,
  ...
  LDBG() << "  scalar sdiv=" << sdiv
         << " vecRange=[" << vecMinS << ", " << vecMaxS << "]";
