...
struct GPUMMASchedule {
  ...
  SmallVector<int64_t> mWarpCounts; // Number of subgroups along M dimension
  SmallVector<int64_t> nWarpCounts; // Number of subgroups along N dimension
  SmallVector<int64_t>
      mTileCounts; // Number of tiles per subgroup along M dimension
  SmallVector<int64_t>
      nTileCounts; // Number of tiles per subgroup along N dimension
  SmallVector<int64_t> kTileCounts; // Number of tiles along K dimension

  GPUMMASchedule(uint64_t i, int64_t m, int64_t n, int64_t k, int64_t mWarp,
                 int64_t nWarp, int64_t mTile, int64_t nTile, int64_t kTile)
      : index(i), mSize(m), nSize(n), kSize(k), mWarpCounts({mWarp}),
        nWarpCounts({nWarp}), mTileCounts({mTile}), nTileCounts({nTile}),
        kTileCounts({kTile}) {}
  GPUMMASchedule(uint64_t i, int64_t m, int64_t n, int64_t k,
                 SmallVector<int64_t> mWarp, SmallVector<int64_t> nWarp,
