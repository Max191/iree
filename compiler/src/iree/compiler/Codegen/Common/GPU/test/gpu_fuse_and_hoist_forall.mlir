...
#map = affine_map<(d0) -> (d0 * 2)>
func.func @no_fuse_collapse_shape_non_equal_subset(%arg0: tensor<8x8xf32>) -> tensor<64xf32> {
  ...
  %1 = scf.forall (%arg1) in (4) shared_outs(%arg2 = %0) -> (tensor<8x8xf32>) {
    ...
    %extracted_slice_0 = tensor.extract_slice %arg2[%arg1, 0] [2, 8] [1, 1] : tensor<8x8xf32> to tensor<2x8xf32>
...
// CHECK-LABEL: func @no_fuse_collapse_shape_non_equal_subset
//       CHECK:   %[[FORALL_RESULT:.+]] = scf.forall {{.*}} -> (tensor<8x8xf32>) {
//       CHECK:     scf.forall.in_parallel {
//   CHECK-DAG:       tensor.parallel_insert_slice {{.*}} : tensor<2x8xf32> into tensor<8x8xf32>
//       CHECK:     }
