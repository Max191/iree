...
struct Im2colOpVectorizationModel
  ...
  bool isVectorizable(Operation *op, ArrayRef<int64_t> vectorSizes,
    ...
    for (int64_t s : outputShape)
      totalElements *= s;
    // Upper bound on unrolled iterations. When no output dimension is
    // vectorized, every element becomes a separate transfer_read/write pair.
    // Exceeding this limit produces IR bloat that slows compilation without
    // proportional runtime benefit.
  ...
  FailureOr<SmallVector<Value>> vectorize(Operation *op, RewriterBase &rewriter,
    ...
    if (hasPadding && vecWidth > 1 && vecDim) {
      // Compute the actual mapped input dim for the vectorized output dim
      // via the input-to-output vectorization map, rather than hardcoding
      // inputRank - 1 which may be incorrect with non-trivial input_k_perm.
      SmallVector<SmallVector<int64_t>> vecMap =
          im2colOp.getInputToOutputDimVectorizationMap();
      int64_t vecInputDim = inputRank - 1;
      for (int64_t d = 0; d < inputRank; ++d) {
        if (llvm::is_contained(vecMap[d], *vecDim)) {
          vecInputDim = d;
          break;
        }
      }
    ...
    // Hoist output-padding check out of the per-iteration loop since it
    // only depends on op attributes, not on the iteration index.
    bool outputPadded =
        hasPadding && IREE::LinalgExt::hasOutputPadding(im2colOp);
    ...
    for (int64_t iter = 0; iter < totalIters; ++iter) {
      ...
      if (mask) {
        ...
        readVec = vector::TransferReadOp::create(
            rewriter, loc, vecType, im2colOp.getInput(), readIndices,
            readPermMap, padValue, /*mask=*/Value(), inBoundsAttr);
      }
