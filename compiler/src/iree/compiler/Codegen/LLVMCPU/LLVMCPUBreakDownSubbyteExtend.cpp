...
// Breaks down extui ops.
//
// This pattern is meant to handle 
//
// ```mlir
...
template <typename T>
static Value shuffleMaskShift(PatternRewriter &rewriter, Location loc, SmallVector<Value> shuffleInputs, int64_t srcBitWidth, int64_t vectorSize) {
  auto shuffleInType = llvm::cast<VectorType>(shuffleInputs[0].getType());
  auto shuffleResultType = VectorType::get({vectorSize}, shuffleInType.getElementType());
  ...
  for (T elemNum = 0; elemNum < shuffleResultType.getNumElements(); elemNum++) {
    maskArray[elemNum] = maskBase
                        << (elemNum * srcBitWidth % dstBitWidth);
  }
  ...
  auto newVectorType = VectorType::get({dstSize}, shuffleResultType.getElementType());
  ...
  for (auto shuffleIn : llvm::enumerate(shuffleInputs)) {
    ...
    for (int64_t elemNum = 0; elemNum < vectorSize;
        elemNum++) {
      shuffleArray[elemNum] = elemNum / (vectorSize /
                                         shuffleInType.getNumElements());
    }
    ...
    newVector = rewriter.create<vector::InsertStridedSliceOp>(loc, shruiResult, newVector, offset, 1);
  }
  return newVector;

}

static std::optional<SmallVector<Value>> getLoadsForExtend(arith::ExtUIOp extOp) {
...
struct BreakDownSubbyteExtend final : OpRewritePattern<arith::ExtUIOp> {
  ...
  LogicalResult matchAndRewrite(arith::ExtUIOp extOp,
    ...
    auto shuffleInputType = VectorType::get({shuffleInputSize}, extuiDstType.getElementType());
    Value shuffleInput = rewriter.create<arith::ConstantOp>(extOp.getLoc(), shuffleInputType, rewriter.getZeroAttr(shuffleInputType));
    ...
    for (int sourceIdx = 0; sourceIdx < sources.size(); sourceIdx++) {
      ...
      for (int64_t i = 0; i < sourceType.getNumElements()/innerSize; i++) {
        ...
        int64_t numElems = i;
        SmallVector<int64_t> sourceOuterShape(sourceShape.begin(), sourceShape.end()-1);
        ...
        Value innerSlice;
        if (indices.size()) {
          innerSlice = rewriter.create<vector::ExtractOp>(extOp.getLoc(), source, indices);
        }
        else {
          innerSlice = source;
        }
        VectorType innerSliceType = llvm::cast<VectorType>(innerSlice.getType());
        int64_t numExtractedBits = innerSliceType.getNumElements() * srcElemBitwidth;
        if (numExtractedBits / dstElemBitwidth < 1) {
          LDBG("extract not big enough: " << numExtractedBits / dstElemBitwidth);
          return failure();
        }
        ...
        if (numExtractedBits >= shuffleInputSizeBits) {
          for (int64_t extractOffset = 0; extractOffset < numExtractedBits / dstElemBitwidth; extractOffset += shuffleInputSize) {
            Value extractedSlice = rewriter.create<vector::ExtractStridedSliceOp>(extOp.getLoc(), bitCastResult, extractOffset, shuffleInputSize, 1);
            shuffleInputs.push_back(extractedSlice);
            LDBG("extractedSlice: " << extractedSlice);
            // vector = rewriter.create<vector::InsertStridedSliceOp>(extOp.getLoc(), extractedSlice, vector, SmallVector<int64_t>{offset}, SmallVector<int64_t>{1});
          }
        }
        else {
          int64_t offset = i * numExtractedBits / dstElemBitwidth % shuffleInputSize;
          shuffleInput = rewriter.create<vector::InsertStridedSliceOp>(extOp.getLoc(), bitCastResult, shuffleInput, SmallVector<int64_t>{offset}, SmallVector<int64_t>{1});
          if (offset + numExtractedBits / dstElemBitwidth == shuffleInputSize) {
            shuffleInputs.push_back(shuffleInput);
            shuffleInput = rewriter.create<arith::ConstantOp>(extOp.getLoc(), shuffleInputType, rewriter.getZeroAttr(shuffleInputType));
          }
        }
      }
    }
    ...
    if (dstElemBitwidth == 32){
      newVector = shuffleMaskShift<int32_t>(rewriter, extOp.getLoc(), shuffleInputs, srcElemBitwidth, vectorSize);
    }
    else if (dstElemBitwidth == 16){
      newVector = shuffleMaskShift<int16_t>(rewriter, extOp.getLoc(), shuffleInputs, srcElemBitwidth, vectorSize);
    }
    rewriter.replaceOpWithNewOp<vector::ShapeCastOp>(extOp, extuiDstType, newVector);
