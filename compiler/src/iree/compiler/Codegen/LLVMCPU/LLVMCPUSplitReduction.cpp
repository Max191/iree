...
LogicalResult splitReductionImpl(Operation *op, SmallVector<int64_t> tileSizes,
  ...
  SmallVector<int64_t> tileSizesSVFirst = tileSizes;
  for (auto &s : tileSizesSVFirst) {
    if (!s) {
      s = 1;
    }
  }
  ...
  LLVM_DEBUG(llvm::dbgs() << "resultCombiningLinalgOp: " << splitRes->resultCombiningLinalgOp << "\n");
