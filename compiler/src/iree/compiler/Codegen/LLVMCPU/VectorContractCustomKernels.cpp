...
MMTKernel MMTKernel_1x2x16_i16i16i32_x86_AVX512VNNI_InlineAsm() {
  MMTKernel kernel;
  ...
  kernel.accRegs = 4;
  kernel.asmImpl = R"ASM(
      vpdpwssd $(acc:0), zmm9, $(lhs:0)
      vpdpwssd $(acc:1), zmm10, $(lhs:0)
      vpdpwssd $(acc:2), zmm11, $(lhs:0)
      vpdpwssd $(acc:3), zmm12, $(lhs:0)
    )ASM";
  kernel.asmClobbers = "";
  return kernel;
}
...
public:
  ...
  LogicalResult matchAndRewrite(vector::ContractionOp contractionOp,
    ...
    for (auto r : lhsRegVectors) {
      LDBG("lhsRegVectors: " << r);
    }
    for (auto r : rhsRegVectors) {
      LDBG("rhsRegVectors: " << r);
    }
    for (auto r : accRegVectors) {
      LDBG("accRegVector: " << r);
    }
    // Generate the kernel!
