...
static LogicalResult fuseDequantAndMatmul(RewriterBase &rewriter,
                                   Operation* dequant,
                                   Operation* matmul){
  Flow::DispatchRegionOp regionOp = matmul->getParentOfType<Flow::DispatchRegionOp>();
  if (!regionOp){
    FailureOr<Flow::DispatchRegionOp> maybeRegionOp = Flow::wrapOpInDispatchRegion(rewriter, matmul);
  ...
  if (failed(maybeFusedRegionOp)) return failure();
...
static LogicalResult isGroupedDequantizationOp(linalg::GenericOp op){
  if (op.getNumResults() != 1) return failure();
  if (op.getNumOperands() != 4) return failure();
...
public:
  ...
  LogicalResult matchAndRewrite(linalg::GenericOp genericOp,
    ...
    if (failed(isMatmulOnGroupedInput(genericOp))) return failure();
    ...
    if (lhsOp && !failed(isGroupedDequantizationOp(llvm::dyn_cast<linalg::GenericOp>(*lhsOp)))){
      if (!failed(fuseDequantAndMatmul(rewriter, lhsOp, matmulOp)))
        succeeded = success();
    }

    if (rhsOp && !failed(isGroupedDequantizationOp(llvm::dyn_cast<linalg::GenericOp>(*rhsOp)))){
