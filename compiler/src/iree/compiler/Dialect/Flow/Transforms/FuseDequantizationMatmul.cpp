...
static LogicalResult ReassociateAndFuseDequantMatmul(
  ...
  // // Fuse ops that quantize the unquantized input into a single dispatch
  // region SmallVector<Operation *> quantizationOps({groupMaxOp,
  // unquantInScalesOp, scaledSumsOp, newQuantInOp});
  // FailureOr<DispatchRegionOp> maybeQuantizationDispatch =
  // wrapConsecutiveOpsInDispatchRegion(rewriter, quantizationOps); if
  // (failed(maybeQuantizationDispatch)) {
  //   return failure();
  // }

  // Fuse ops that perform the dequantization + matmul into a single dispatch
  // region
  ...
  // for (auto op :
  // maybeDequantMatmulDispatch.value().getBody().getOps<linalg::GenericOp>()) {
...
//   LogicalResult matchAndRewrite(linalg::GenericOp genericOp, PatternRewriter
