...
struct FuseEncodingOpsIntoDispatchRegionsPass
  ...
  void runOnOperation() override {
    ...
    funcOp->walk([&](IREE::Encoding::SetEncodingOp encodingOp) {
      ...
      // group root, but set to the full rank of the SetEncoding for now to
      // be conservative.
      auto outerParallelLoops =
          llvm::SmallBitVector(encodingOp.getResultType().getRank(), true);
