...
static bool willBeContiguousSlice(OpFoldResult inputSize,
  ...
  return *constInputSize % *tileSizeUB == 0 &&
         offsetDiv % *tileSizeUB == 0;
}

static std::optional<int64_t>
chooseDimToVectorize(OpBuilder &b, Location loc,
                     Im2colOp im2colOp,
  ...
  SmallVector<int64_t> vectorizableOutputDims =
      vectorizationMap[innerInputDim];
  if (vectorizableOutputDims.empty()) {
    return std::nullopt;
  }
  SmallVector<int64_t> kOutputDims = im2colOp.getKOutputDims();
  SetVector<int64_t> kDimSet(kOutputDims.begin(),
                             kOutputDims.end());
  ...
  while (!vectorizableOutputDims.empty()) {
    int64_t outputDimToVectorize =
        vectorizableOutputDims.pop_back_val();
    if (outputDimToVectorize ==
        im2colOp.getBatchOutputDims().back()) {
    ...
    SmallVector<int64_t> mOutputDims =
        im2colOp.getMOutputDims();
    SetVector<int64_t> mDimSet(llvm::from_range,
                               mOutputDims);
    ...
    OpFoldResult outputDimSize =
        iterationDomain[outputDimToVectorize].size;
    if (!willBeContiguousSlice(innerSliceSize,
                               outputDimSize, offset,
                               solver)) {
      continue;
    }
    return outputDimToVectorize;
...
static std::optional<int64_t> getDimBound(Value v,
                                          int64_t dim) {
  auto ub = ValueBoundsConstraintSet::computeConstantBound(
      presburger::BoundType::UB, {v, dim}, nullptr, true);
  if (failed(ub)) {
    return std::nullopt;
  }
  return ub.value();
...
static void annotateIm2colVectorizationHints(
  ...
  funcOp->walk([&](Im2colOp im2colOp) {
    ...
    rewriter.setInsertionPoint(im2colOp);
    SmallVector<OpFoldResult> inputSizes =
        tensor::getMixedSizes(rewriter, loc,
                              im2colOp.getInput());
    ...
    for (int64_t dim = 0;
         dim < im2colOp.getOutputRank(); ++dim) {
      std::optional<int64_t> dimBound =
          getDimBound(im2colOp.getOutput(), dim);
    ...
    std::optional<int64_t> dimToVectorize =
        chooseDimToVectorize(rewriter, loc, im2colOp,
                             iterationDomain, inputSizes,
                             kOffset, solver);
    if (dimToVectorize.has_value()) {
      dimsToVectorize.insert(dimToVectorize.value());
    }
    im2colOp.setVectorizationHint(
        dimsToVectorize.getArrayRef());
    LDBG() << "annotated im2col with vectorization hint: "
           << im2colOp << "\n";
  });
}
...
void DecomposeIm2colPass::runOnOperation() {
  ...
  // Simplify affine min/max before decomposition.
  {
    IRRewriter rewriter(context);
    SmallVector<Operation *> affineOps;
    funcOp->walk([&](Operation *op) {
      if (isa<affine::AffineMinOp,
              affine::AffineMaxOp>(op)) {
        affineOps.push_back(op);
      }
    });
    if (!affineOps.empty()) {
      (void)affine::simplifyAffineMinMaxOps(
          rewriter, affineOps);
    }
  }

  // Run divisibility analysis and annotate vectorization hints.
  DataFlowSolver solver;
  ...
  // Decompose im2col ops.
  SmallVector<Im2colOp> candidates;
  funcOp->walk(
      [&](Im2colOp op) { candidates.push_back(op); });
  IRRewriter rewriter(context);
  for (auto im2colOp : candidates) {
    if (failed(
            decomposeIm2col(im2colOp, rewriter, unroll))) {
      return signalPassFailure();
    }
  }

  // Cleanup: fold reshapes by expansion around pad ops,
  // swap extract_slice with collapse_shape, and compose
  // pad ops.
  auto populateCleanupPatterns =
      [&context](RewritePatternSet &patterns) {
        linalg::FillOp::getCanonicalizationPatterns(
            patterns, context);
        tensor::CollapseShapeOp::getCanonicalizationPatterns(
            patterns, context);
        tensor::EmptyOp::getCanonicalizationPatterns(
            patterns, context);
        tensor::ExpandShapeOp::getCanonicalizationPatterns(
            patterns, context);
        tensor::PadOp::getCanonicalizationPatterns(
            patterns, context);
        memref::
...

  {
    ...
    linalg::ControlFusionFn expandControlFn =
        [](OpOperand *fusedOperand) {
          Operation *producer =
              fusedOperand->get().getDefiningOp();
          Operation *consumer = fusedOperand->getOwner();
          return isa<tensor::PadOp>(producer) ||
                 isa<tensor::PadOp>(consumer);
        };
    linalg::populateFoldReshapeOpsByExpansionPatterns(
        expandPatterns, expandControlFn);
    populateCleanupPatterns(expandPatterns);
    expandPatterns
        .insert<linalg::ExtractSliceOfPadTensorSwapPattern>(
            context,
            [](tensor::ExtractSliceOp) { return false; });
    if (failed(applyPatternsGreedily(
            getOperation(),
            std::move(expandPatterns)))) {
      return signalPassFailure();
    }
  }

  {
    ...
    linalg::ControlFusionFn collapseControlFn =
        [](OpOperand *fusedOperand) {
          Operation *producer =
              fusedOperand->get().getDefiningOp();
          Operation *consumer = fusedOperand->getOwner();
    ...
    linalg::populateFoldReshapeOpsByCollapsingPatterns(
        collapsePatterns, collapseControlFn);
    populateCleanupPatterns(collapsePatterns);
    if (failed(applyPatternsGreedily(
            getOperation(),
            std::move(collapsePatterns)))) {
      return signalPassFailure();
    }
  }
}
