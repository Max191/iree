...
// Test: output pad on K dimension should NOT be folded.
// Padding K is semantically incorrect — extra K positions would pollute GEMM
// accumulation for valid M positions.

func.func @no_fold_output_pad_k_padding(%arg0: tensor<2x34x34x640xf32>) -> tensor<2x1024x5776xf32> {
  %cst = arith.constant 0.0 : f32
  %empty = tensor.empty() : tensor<2x1024x5760xf32>
  %result = iree_linalg_ext.im2col
      strides = [1, 1] dilations = [1, 1] kernel_size = [3, 3]
...
// CHECK-LABEL: func.func @no_fold_output_pad_k_padding(
//       CHECK:   iree_linalg_ext.im2col
//       CHECK:   tensor.pad

// -----

// Test: output pad with K padding should NOT be folded even when M is also
// padded. The K-dim guard must reject any non-zero K padding regardless of
// other dimensions.

func.func @no_fold_output_pad_k_and_m_padding(%arg0: tensor<2x34x34x640xf32>) -> tensor<2x1040x5824xf32> {
  %cst = arith.constant 0.0 : f32
  %empty = tensor.empty() : tensor<2x1024x5760xf32>
  %result = iree_linalg_ext.im2col
      strides = [1, 1] dilations = [1, 1] kernel_size = [3, 3]
...
// CHECK-LABEL: func.func @no_fold_output_pad_k_and_m_padding(
//       CHECK:   iree_linalg_ext.im2col
//       CHECK:   tensor.pad

// -----

// Test: output pad should NOT be folded when im2col has no input padding.
// Without padding attributes, the vectorization path won't emit result masks,
// so OOB positions in the enlarged output get garbage values.

func.func @no_fold_output_pad_no_input_padding(%arg0: tensor<2x34x34x640xf32>) -> tensor<2x1040x5760xf32> {
  %cst = arith.constant 0.0 : f32
  %empty = tensor.empty() : tensor<2x1024x5760xf32>
  %result = iree_linalg_ext.im2col
      strides = [1, 1] dilations = [1, 1] kernel_size = [3, 3]
...
// CHECK-LABEL: func.func @no_fold_output_pad_no_input_padding(
//       CHECK:   iree_linalg_ext.im2col
//       CHECK:   tensor.pad
