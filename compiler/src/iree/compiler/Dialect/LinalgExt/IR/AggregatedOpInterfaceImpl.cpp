...
FailureOr<SmallVector<Value>> Im2colOp::decomposeOperation(OpBuilder &b) {
  ...
  std::optional<int64_t> maybeOutputDimToVectorize = chooseDimToVectorize(
      b, loc, *this, iterationDomain, mixedOffsets);
  ...
  Value validSize;
  if (hasPadding()) {
    validSize = computeIm2colValidSize(
        b, nestedLoc, *this, srcIndices, innerInputTileSize, ivs,
  ...
  if (!hasPadding()) {
    ...
    Value highPadAmt =
        arith::SubIOp::create(b, nestedLoc, tileSize, validSize);
