...
FailureOr<SmallVector<Value>> Im2colOp::decomposeOperation(OpBuilder &b) {
  ...
  std::optional<int64_t> maybeOutputDimToVectorize = chooseDimToVectorize(
  ...
  // Step 2: When vectorizing a dim, insert a zero IV for it (it spans the full tile).
  if (maybeOutputDimToVectorize.has_value()) {
  ...
  Im2colSourceIndices srcIndices = computeIm2colSourceIndices(
      b, nestedLoc, *this, ivs, innerInputTileSize);
  ...
  // Compute output insertion offsets and sizes (shared by both paths).
  ...
  // slice. Two paths based on padding status:
  //  - No padding: skip bounds IR, use srcIndices directly (fast path).
  //  - Padded: bounds IR + tensor.pad with pad_value. Output-only padding
  //    (no pad_value) produces a degenerate tensor.pad that canonicalizes away.
  // Output padding (from FoldOutputPadIntoIm2col) only applies to padded ops.
  // Skip the check for non-padded ops to avoid conservative false positives
  // from dynamic offsets/shapes that would emit unnecessary clamping IR.
  bool outputPadded = hasPadding() && hasOutputPadding(*this);
  ...
  if (!hasPadding()) {
    ...
    // Padded path: dynamic extract + tensor.pad.
