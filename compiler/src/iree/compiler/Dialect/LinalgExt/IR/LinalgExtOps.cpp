...
static void printOptionalPadding(OpAsmPrinter &p, Operation *op,
                                 ...
                                 OperandRange outputPadLow,
                                 DenseI64ArrayAttr staticOutputPadLow,
                                 OperandRange outputPadHigh,
                                 DenseI64ArrayAttr staticOutputPadHigh,
                                 Value padValue, Type padValueType) {
  if (!padValue) {
    return;
  }
  // Print input padding only if present (non-empty).
  if (!staticInputPadLow.empty()) {
    p << "input_pad_low = ";
  ...
  // Print output padding only if present (non-empty).
  if (!staticOutputPadLow.empty()) {
    p << "output_pad_low = ";
    printDynamicIndexList(p, op, outputPadLow, staticOutputPadLow.asArrayRef());
    p << " output_pad_high = ";
    printDynamicIndexList(p, op, outputPadHigh,
                          staticOutputPadHigh.asArrayRef());
    p << " ";
  }
...
/// Parses one of:
///   input_pad_low = [...] input_pad_high = [...]
///   output_pad_low = [...] output_pad_high = [...]
///   pad_value(%cst : f32)
/// or various combinations (input only, output only, both, or neither).
/// pad_value is always last when present.
static ParseResult parseOptionalPadding(
    ...
    DenseI64ArrayAttr &staticInputPadHigh,
    SmallVectorImpl<OpAsmParser::UnresolvedOperand> &outputPadLow,
    DenseI64ArrayAttr &staticOutputPadLow,
    SmallVectorImpl<OpAsmParser::UnresolvedOperand> &outputPadHigh,
    DenseI64ArrayAttr &staticOutputPadHigh,
    std::optional<OpAsmParser::UnresolvedOperand> &padValue,
    Type &padValueType) {
  // Helper to parse pad_value(%val : type).
  auto parsePadValue = [&]() -> ParseResult {
  ...
  // Helper to set empty defaults for input padding.
  auto setEmptyInputPad = [&]() {
    staticInputPadLow =
  ...
  // Helper to set empty defaults for output padding.
  auto setEmptyOutputPad = [&]() {
    staticOutputPadLow =
        DenseI64ArrayAttr::get(parser.getContext(), ArrayRef<int64_t>{});
    staticOutputPadHigh =
        DenseI64ArrayAttr::get(parser.getContext(), ArrayRef<int64_t>{});
  ...
  // Try to parse input_pad_low first.
  if (succeeded(parser.parseOptionalKeyword("input_pad_low"))) {
    ...
    // Optionally parse output_pad_low/high after input padding.
    if (succeeded(parser.parseOptionalKeyword("output_pad_low"))) {
      if (parser.parseEqual() ||
          parseDynamicIndexList(parser, outputPadLow, staticOutputPadLow)) {
        return failure();
      }
      if (parser.parseKeyword("output_pad_high") || parser.parseEqual() ||
          parseDynamicIndexList(parser, outputPadHigh, staticOutputPadHigh)) {
        return failure();
      }
    } else {
      setEmptyOutputPad();
    ...
    if (parser.parseKeyword("pad_value") || parsePadValue()) {
      return failure();
    }
    return success();
  }

  // Try output_pad_low without input padding.
  if (succeeded(parser.parseOptionalKeyword("output_pad_low"))) {
    setEmptyInputPad();
    if (parser.parseEqual() ||
        parseDynamicIndexList(parser, outputPadLow, staticOutputPadLow)) {
  ...
  // Try to parse pad_value without any explicit padding.
  if (succeeded(parser.parseOptionalKeyword("pad_value"))) {
    setEmptyInputPad();
    setEmptyOutputPad();
    if (parsePadValue()) {
      return failure();
...
void Im2colOp::build(OpBuilder &builder, OperationState &state, Value input,
                     ...
                     ArrayRef<OpFoldResult> inputPadLow,
                     ArrayRef<OpFoldResult> inputPadHigh,
                     ArrayRef<OpFoldResult> outputPadLow,
                     ArrayRef<OpFoldResult> outputPadHigh, Value padValue) {
...
struct FoldOutputPadIntoIm2col final : public OpRewritePattern<tensor::PadOp> {
  ...
  LogicalResult matchAndRewrite(tensor::PadOp padOp,
    ...
    // This fold is safe for K-dimension padding because im2col always fills
    // padded K positions with pad_value (typically zero). Zero pad value means
    // extra K positions contribute 0 to MAC accumulation in the downstream
    // GEMM, preserving correctness.
    ...
    if (!existingOutPadLow.empty()) {
      for (int64_t i = 0; i < outputRank; ++i) {
        newOutPadLow[i] = addOfrs(rewriter, loc, existingOutPadLow[i],
                                  newOutPadLow[i]);
        newOutPadHigh[i] = addOfrs(rewriter, loc, existingOutPadHigh[i],
                                   newOutPadHigh[i]);
      }
    }
    ...
    auto newIm2col = Im2colOp::create(
        ...
        im2colOp.getMixedInputPadHigh(), newOutPadLow, newOutPadHigh,
        padValue);
