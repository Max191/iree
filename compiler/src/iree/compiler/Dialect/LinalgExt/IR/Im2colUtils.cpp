...
Im2colSourceIndices computeIm2colSourceIndices(OpBuilder &b, Location loc,
  ...
  for (int64_t d = 0; d < numCanonicalDims; ++d) {
    ...
    if (numProduced == 1) {
      ...
      // Pass all basis elements with hasOuterBound=true. This enables the
      // DropUnitExtentBasis canonicalizer to eliminate unit-extent dimensions
      // (e.g., a leading "1" in [1, 59, 91]) by replacing them with constant
      // 0 and removing the corresponding division.
...
Value computeIm2colValidSize(OpBuilder &b, Location loc, Im2colOp im2colOp,
  ...
  auto remainingValid = [&](OpFoldResult extent, OpFoldResult coord,
    ...
    OpFoldResult rawValid = affine::makeComposedFoldedAffineApply(
        b, loc, subMap, {extent, coord});
  ...
  OpFoldResult validSizeOfr =
      remainingValid(inputSizes[vecInputDim],
                     adjustedOffsets[vecInputDim], innerTileSize);
  ...
  if (!outPadLow.empty()) {
    ...
    if (vecOutputDim.has_value()) {
      ...
      if (!isConstantIntValue(outPadHigh[vd], 0)) {
        ...
        OpFoldResult vsOfr =
            remainingValid(validEnd, outputIVs[vd], validSize);
