...
LogicalResult MapScatterOp::generateScalarImplementation(OpBuilder &b,
  ...
  auto bodyBuilder = [&](OpBuilder nestedBuilder, Location nestedLoc,
    ...
    auto thenBuilder = [&](OpBuilder &ifBuilder, Location ifLoc) {
      ...
      Operation *input =
          isa<VectorType>(getInputType())
              ? ifBuilder.create<vector::ExtractOp>(ifLoc, getInput(), ivsOfr)
              : ifBuilder.create<memref::LoadOp>(ifLoc, getInput(), ivs);
      ifBuilder.create<memref::StoreOp>(ifLoc, input->getResult(0), getOutput(),
                                        storeIndices);
