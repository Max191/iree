...
// Test: tensor.pad with constant padding and static offsets feeding a
// conv-like linalg.generic with reduction dims → raised to map_load.

// CHECK-LABEL: util.func public @raise_pad_to_map_load
//  CHECK-SAME:     %[[SRC:.+]]: tensor<1x4x4x2xf32>
//  CHECK-SAME:     %[[FILTER:.+]]: tensor<2x3x3x2xf32>
//   CHECK-DAG:   %[[CST:.+]] = arith.constant 0.000000e+00 : f32
//   CHECK-DAG:   %[[C1:.+]] = arith.constant 1 : index
//       CHECK:   %[[EMPTY:.+]] = tensor.empty() : tensor<1x6x6x2xf32>
//       CHECK:   %[[ML:.+]] = iree_linalg_ext.map_load %[[SRC]] into %[[EMPTY]]
//       CHECK:     ^bb0(%[[I0:.+]]: index, %[[I1:.+]]: index, %[[I2:.+]]: index, %[[I3:.+]]: index):
//       CHECK:       %[[ADJ1:.+]] = arith.subi %[[I1]], %[[C1]] : index
//       CHECK:       %[[ADJ2:.+]] = arith.subi %[[I2]], %[[C1]] : index
//       CHECK:       iree_linalg_ext.yield %[[I0]], %[[ADJ1]], %[[ADJ2]], %[[I3]], %[[CST]]
//       CHECK:   linalg.generic
//  CHECK-SAME:     ins(%[[ML]], %[[FILTER]]
//       CHECK:   util.return
util.func public @raise_pad_to_map_load(%arg0: tensor<1x4x4x2xf32>, %arg1: tensor<2x3x3x2xf32>) -> tensor<1x4x4x2xf32> {
  %cst = arith.constant 0.000000e+00 : f32
  %padded = tensor.pad %arg0 low[0, 1, 1, 0] high[0, 1, 1, 0] {
  ^bb0(%a0: index, %a1: index, %a2: index, %a3: index):
    tensor.yield %cst : f32
  } : tensor<1x4x4x2xf32> to tensor<1x6x6x2xf32>
  %init = tensor.empty() : tensor<1x4x4x2xf32>
  %fill = linalg.fill ins(%cst : f32) outs(%init : tensor<1x4x4x2xf32>) -> tensor<1x4x4x2xf32>
  ...
  } ins(%padded, %arg1 : tensor<1x6x6x2xf32>, tensor<2x3x3x2xf32>)
    outs(%fill : tensor<1x4x4x2xf32>) {
  ^bb0(%in0: f32, %in1: f32, %out: f32):
    %mul = arith.mulf %in0, %in1 : f32
    %add = arith.addf %out, %mul : f32
    linalg.yield %add : f32
  } -> tensor<1x4x4x2xf32>
  util.return %result : tensor<1x4x4x2xf32>
}

// -----

// Test: tensor.pad with dynamic low pad values feeding a reduction consumer.
// Verify that dynamic offsets appear in arith.subi.

// CHECK-LABEL: util.func public @raise_pad_dynamic_offsets
//  CHECK-SAME:     %[[SRC:[a-zA-Z0-9]+]]: tensor<8x8xf32>
//  CHECK-SAME:     %[[LOW0:[a-zA-Z0-9]+]]: index
//  CHECK-SAME:     %[[LOW1:[a-zA-Z0-9]+]]: index
//   CHECK-DAG:   %[[CST:.+]] = arith.constant 0.000000e+00 : f32
//       CHECK:   %[[EMPTY:.+]] = tensor.empty(%{{.*}}, %{{.*}}) : tensor<?x?xf32>
//       CHECK:   %[[ML:.+]] = iree_linalg_ext.map_load %[[SRC]] into %[[EMPTY]]
//       CHECK:     ^bb0(%[[I0:.+]]: index, %[[I1:.+]]: index):
//       CHECK:       %[[ADJ0:.+]] = arith.subi %[[I0]], %[[LOW0]] : index
//       CHECK:       %[[ADJ1:.+]] = arith.subi %[[I1]], %[[LOW1]] : index
//       CHECK:       iree_linalg_ext.yield %[[ADJ0]], %[[ADJ1]], %[[CST]]
//       CHECK:   linalg.generic
//  CHECK-SAME:     ins(%[[ML]]
util.func public @raise_pad_dynamic_offsets(%arg0: tensor<8x8xf32>, %low0: index, %low1: index) -> tensor<?xf32> {
  %cst = arith.constant 0.000000e+00 : f32
  %high0 = arith.constant 2 : index
  %high1 = arith.constant 2 : index
  %padded = tensor.pad %arg0 low[%low0, %low1] high[%high0, %high1] {
  ^bb0(%a0: index, %a1: index):
    tensor.yield %cst : f32
  } : tensor<8x8xf32> to tensor<?x?xf32>
  %c0 = arith.constant 0 : index
  %d0 = tensor.dim %padded, %c0 : tensor<?x?xf32>
  ...
  } ins(%padded : tensor<?x?xf32>) outs(%fill : tensor<?xf32>) {
  ^bb0(%in: f32, %out: f32):
...
// Negative test: pad with non-constant padding value (index-dependent).
// Should NOT be raised to map_load.

// CHECK-LABEL: util.func public @no_raise_pad_non_constant_value
//       CHECK:   tensor.pad
//       CHECK:   linalg.generic
util.func public @no_raise_pad_non_constant_value(%arg0: tensor<4x4xf32>) -> tensor<6xf32> {
  %cst = arith.constant 0.000000e+00 : f32
  %padded = tensor.pad %arg0 low[1, 1] high[1, 1] {
  ^bb0(%idx0: index, %idx1: index):
    %v = arith.index_castui %idx0 : index to i32
    %f = arith.sitofp %v : i32 to f32
    tensor.yield %f : f32
  } : tensor<4x4xf32> to tensor<6x6xf32>
  %init = tensor.empty() : tensor<6xf32>
  %fill = linalg.fill ins(%cst : f32) outs(%init : tensor<6xf32>) -> tensor<6xf32>
  %result = linalg.generic {
    indexing_maps = [
      affine_map<(d0, d1) -> (d0, d1)>,
      affine_map<(d0, d1) -> (d0)>],
    iterator_types = ["parallel", "reduction"]
  } ins(%padded : tensor<6x6xf32>) outs(%fill : tensor<6xf32>) {
  ^bb0(%in: f32, %out: f32):
    %add = arith.addf %in, %out : f32
    linalg.yield %add : f32
  } -> tensor<6xf32>
  util.return %result : tensor<6xf32>
}

// -----

// Negative test: pad feeding only an elementwise consumer (no reduction dims).
// Should NOT be raised to map_load.

// CHECK-LABEL: util.func public @no_raise_pad_elementwise_consumer
//       CHECK:   tensor.pad
//       CHECK:   linalg.generic
util.func public @no_raise_pad_elementwise_consumer(%arg0: tensor<4x4xf32>) -> tensor<6x6xf32> {
  %cst = arith.constant 0.000000e+00 : f32
  %padded = tensor.pad %arg0 low[1, 1] high[1, 1] {
  ^bb0(%a0: index, %a1: index):
    tensor.yield %cst : f32
  ...
  %result = linalg.generic {
    indexing_maps = [
      ...
      affine_map<(d0, d1) -> (d0, d1)>],
    iterator_types = ["parallel", "parallel"]
  } ins(%padded : tensor<6x6xf32>) outs(%init : tensor<6x6xf32>) {
  ^bb0(%in: f32, %out: f32):
    %neg = arith.negf %in : f32
    linalg.yield %neg : f32
  } -> tensor<6x6xf32>
  util.return %result : tensor<6x6xf32>
}
