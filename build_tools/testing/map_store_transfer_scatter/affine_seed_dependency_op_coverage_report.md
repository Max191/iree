...
Loop-like induction-value support is now covered by checked-in lit tests in
`compiler/src/iree/compiler/Dialect/Util/Transforms/test/test_affine_seed_dependency_analysis.mlir`:

- `@map_store_capture_scf_for_iv` checks that a captured `scf.for` induction
  value is independent of map-store region seeds (`seed0 = 0, seed1 = 0`) and
  that `%idx1 + %iv` preserves the `%idx1` coefficient (`seed1 = 1`).
- `@map_store_capture_scf_forall_iv` checks the same behavior for a captured
  `scf.forall` induction value.
- `@scf_for_iv_lower_bound_seed` and `@scf_for_iv_seed_dependent_step` cover
  the loop-IV transfer rule itself: seed-dependent lower bounds propagate a
  coefficient of `1`, while seed-dependent steps conservatively produce a
  per-seed unknown coefficient.

Remaining implicit-capture inventory work is therefore about real-pipeline
coverage, not basic `scf.for` / `scf.forall` support: collect the producer
slice for captured values in generated BOO/data-tiling/e2e dumps and check
whether additional producer ops need interface support.
